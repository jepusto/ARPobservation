#internal function representing the probability that an event will be occuring x seconds into the future, given that no event is currently ocurring
p_0 <- function(x,phi,zeta) phi * (1- exp(-1 * x * zeta / (phi * (1- phi))))
#internal function representing the probability that an event will be occuring x seconds into the future, given that an event is currently occuring
...
#' 
#' @param phi  proposed value for prevalence
#' @param zeta proposed value for incidence
#' @param U a vector containing the session-level data for a PIR observation
#' @param c the length of the active interval
#' @param d the length of the recording interval
#' 
#' @details \code{phi} must be a value between 0 and 1, inclusive of 0 and 1. \code{zeta} must be a value greater than 0.
#' 
#' The vector \code{U} should only contain values of 1 or 0.
#' 
#' \code{c} must be some positive value in whatever time units the observation took place in (typically seconds). \code{d} must be some non-negative value - a \code{d} of zero represents a PIR observation where no time was set aside for recording.
#' 
...
PIR_loglik <- function(phi, zeta, U, c, d){
  ...
  if(phi < 0 | phi > 0){stop("Values for phi must be between 0 and 1, inclusive")}
